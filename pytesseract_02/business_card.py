...
def upload():
    ...
    image_nparray = np.asarray(bytearray(requests.get(file_upload).content), dtype=np.uint8)
    print("드라이브에서 가져온 사진 ===>", file_upload)
...
#
#
#plt_imshow("orignal image", upload())
#
...
#
#receipt_image = make_scan_image(upload(), width=200, ksize=(5, 5), min_threshold=20, max_threshold=100)
#
#
#
#
...
#
## options = "--psm 4"
## text = pytesseract.image_to_string(cv2.cvtColor(receipt_image, cv2.COLOR_BGR2RGB), config=options, lang='kor+eng')
# 
